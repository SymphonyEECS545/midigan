...
def generation_test(sess, dcgan, config, option):
    if option == 0:
    

        sample_labels = sloppy_sample_labels()
        #prev_batch_images = np.random.randint(low=48,high=72,size = [72, 16, 128, 1])
...
def piano_roll_to_pretty_midi(piano_roll, sf=16, program_num=0):
